...
SocketServer::SocketServer() : bStart(false), iFD(-1) {}
...
bool SocketServer::Accept(int& iClientFD) {
	...
	iClientFD =
		accept(this->iFD, (struct sockaddr*)&sSockaddr, (socklen_t*)&iSizeofSockaddr);
	if (iClientFD != -1) {
		return true;
	}
...
bool SocketServer::Start(
	const int& iPort, const int iTimeOut, const int iJobPoolSize,
	...
	while (this->bStart) {
		...
		auto jobFinal = [=]() { jobFunc(SocketClient(iClientFD, iTimeOut)); };
