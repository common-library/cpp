...
class EnvironmentVariableTest : public ::testing::Test {
	protected:
		...
		void SetUp() override {
			...
			int iArgc = 4;
			char* pcArgv[] = {(char*)"./EnvironmentVariableTest", (char*)"-c",
...
TEST_F(EnvironmentVariableTest, Usage) {
	EXPECT_STREQ(this->environmentVariable.Usage().c_str(),
				 "usage: EnvironmentVariableTest -c config_path [-s]");
}
...
TEST_F(EnvironmentVariableTest, GetConfigPath) {
	EXPECT_STREQ(this->environmentVariable.GetConfigPath().c_str(),
				 GstrConfigPath.c_str());
}

TEST_F(EnvironmentVariableTest, GetProcessName) {
	EXPECT_STREQ(this->environmentVariable.GetProcessName().c_str(),
...
TEST_F(EnvironmentVariableTest, GetProcess) {
	...
	EXPECT_EQ(&parentProcess,
			  this->environmentVariable.GetProcess(E_PROCESS_TYPE::PARENT));
}
