...
protected:
	void SetUp() override {
		...
		EXPECT_FALSE(Singleton<EnvironmentVariable>::Instance().Initialize(iArgc, pcArgv));
	}
...
protected:
	void SetUp() override {
		...
		int iArgc = 4;
		char *pcArgv[] = {(char *)"./StandaloneChildProcessTest", (char *)"-c", (char *)GstrConfigPath.c_str(), (char *)"-s"};
...
protected:
	void SetUp() override {
		...
		int iArgc = 3;
		char *pcArgv[] = {(char *)"./NonStandaloneChildProcessTest", (char *)"-c", (char *)GstrConfigPath.c_str()};
...
TEST_F(NonStandaloneChildProcessTest, Start)
{
	...
	future<bool> future = threadPool.AddJob([&]()->bool{return testChildProcess.Start();});
	this_thread::sleep_for(chrono::seconds(1));
