...
enum class E_PARSING_TYPE { FILE, STRING };

static void test_json1(const E_PARSING_TYPE& eParsingType,
					   const JsonManager& jsonManager) {
	switch (eParsingType) {
...
static void test_json2(const E_PARSING_TYPE& eParsingType,
					   const JsonManager& jsonManager) {
	switch (eParsingType) {
	case E_PARSING_TYPE::FILE: {
		const string strTemplate = FileManager().GetTempPath() + "/tmpXXXXXX";
