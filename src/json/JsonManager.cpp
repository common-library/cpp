...
JsonManager::JsonManager(const E_JSON_PARSER &eJsonParser)
	: eJsonParser(eJsonParser),
		mapJson({
					{E_JSON_PARSER::BOOST, new BoostJson()},
...
vector<map<string, JSON_VALUE_TYPE>> JsonManager::GetArray(const vector<string> &vecKey, const string &strArrayName, const map<string, JSON_VALUE_TYPE> &mapValueInfo) const
{
	return this->mapJson.at(this->eJsonParser)->GetArray(vecKey, strArrayName, mapValueInfo);
}
