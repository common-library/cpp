...
bool CommonConfig::InitializeDerived()
{
	...
	this->strLogFileNamePrefix = this->jsonManager.GetValue<string>({"log_file_name_prefix"});

	return true;
}

string CommonConfig::GetWorkingPath()
{
	return this->strWorkingPath;
}

E_LOG_LEVEL CommonConfig::GetLogLevel()
{
	return this->eLogLevel;
}

bool CommonConfig::GetThreadMode()
{
	return this->bThreadMode;
}

string CommonConfig::GetLogOutputPath()
{
	return this->strLogOutputPath;
}

string CommonConfig::GetLogFileNamePrefix()
{
	return this->strLogFileNamePrefix;
}
